...
SimulatorInit::SimulatorInit(InertialInitializerOptions &params_) {
  ...
  for (int i = 0; i < params.num_cameras; i++) {
    ...
    while (true) {
      ...
      std::vector<std::pair<size_t, Eigen::Vector2f>> uvs = project_pointcloud(R_GtoI.cast<double>(), p_IinG.cast<double>(), i, featmap);
      // If we do not have enough, generate more
      if ((int)uvs.size() < params.init_max_features) {
        generate_points(R_GtoI.cast<double>(), p_IinG.cast<double>(), i, featmap, params.init_max_features - (int)uvs.size());
      }
...
bool SimulatorInit::get_state(double desired_time, Eigen::Matrix<DataType, 17, 1> &imustate) {
  ...
  imustate.block(11, 0, 3, 1) = true_bg_interp.cast<float>();
  imustate.block(14, 0, 3, 1) = true_ba_interp.cast<float>();
  return true;
}

bool SimulatorInit::get_next_imu(double &time_imu, Vec3 &wm, Vec3 &am) {
...
SimulatorInit::project_pointcloud(const Eigen::Matrix3d &R_GtoI, const Eigen::Vector3d &p_IinG, int camid,
  ...
  for (const auto &feat : feats) {
    ...
    uv_dist = camera->distort(uv_norm);
