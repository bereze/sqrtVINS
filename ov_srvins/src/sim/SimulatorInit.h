...
public:
   ...
   */
  bool get_state(double desired_time, Eigen::Matrix<DataType, 17, 1> &imustate);
   ...
   * @return True if we have a measurement
   */
  bool get_next_cam(double &time_cam, std::vector<int> &camids, std::vector<std::vector<std::pair<size_t, Eigen::Vector2f>>> &feats);
...
protected:
   ...
   */
  std::vector<std::pair<size_t, Eigen::Vector2f>> project_pointcloud(const Eigen::Matrix3d &R_GtoI, const Eigen::Vector3d &p_IinG,
